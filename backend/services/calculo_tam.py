...
async def obter_resultados_tam(
    job_id: str, db: AsyncIOMotorDatabase
) -> List[Dict]:
...
async def ranking_tam(
    resultados: list[TamResponse], top_n: int = 10
) -> list[TamResponse]:
    """
    ...
    ranking_completo = sorted(
        resultados, key=lambda x: x.COMP_KM, reverse=True
    )
