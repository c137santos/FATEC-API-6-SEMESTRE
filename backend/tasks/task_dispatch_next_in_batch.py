...
def task_dispatch_next_in_batch(batch_id: str | None = None) -> dict:
    ...
    try:
        ...
        if not batch:
            logger.warning(
                '[task_dispatch_next_in_batch] Batch não encontrado. batch_id=%s',
                batch_id,
            )
            return {'skipped': True}
        ...
        pending = [
            d
            for d in batch.get('distribuidoras', [])
            if d['status'] == 'pending'
        ]
        if not pending:
            logger.info(
                '[task_dispatch_next_in_batch] Sem pendentes. batch_id=%s',
                batch_id,
            )
            return {'done': True}
        ...
        try:
            ...
            if _update_batch_dist_status(
                db, batch_id, next_item['id'], 'skipped', str(exc)
            ):
                task_dispatch_next_in_batch.apply_async(args=[batch_id])
            return {'skipped': next_item['id']}
        except Exception as exc:
            logger.exception(
                '[task_dispatch_next_in_batch] Falha ao disparar. dist_id=%s',
                next_item['id'],
            )
            if _update_batch_dist_status(
                db, batch_id, next_item['id'], 'failed', str(exc)
            ):
                task_dispatch_next_in_batch.apply_async(args=[batch_id])
        ...
        logger.exception(
            '[task_dispatch_next_in_batch] Erro inesperado, reagendando. batch_id=%s',
            batch_id,
        )
        try:
            task_dispatch_next_in_batch.apply_async(
                args=[batch_id], countdown=60
            )
        except Exception:
            logger.exception(
                '[task_dispatch_next_in_batch] Falha ao reagendar. batch_id=%s',
                batch_id,
            )
