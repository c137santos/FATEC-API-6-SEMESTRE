...
class TestSumColumns:
    def test_soma_valores_presentes(self):
    ...
    def test_record_vazio_retorna_zero(self):
        assert _sum_columns({}, PT_COLUMNS) == 0.0


class TestCalculatePtPnt:
    def _patch_mongo(self, mock_client):
        return patch(
            'backend.core.calculate_pt_and_pnt.MongoClient',
            return_value=mock_client,
        )

    def test_retorna_lista_com_conjunto(self):
        ...
        with self._patch_mongo(mock_client):
    ...
    def test_calcula_percentuais_corretamente(self):
    ...
    def test_pct_none_quando_pt_e_pnt_zerados(self):
    ...
    def test_agrega_multiplos_circuitos_no_mesmo_conjunto(self):
        ctmt = [
    ...
    def test_dois_conjuntos_distintos(self):
        ctmt = [
    ...
    def test_ordenado_por_pnt_decrescente(self):
        ctmt = [
    ...
    def test_retorna_lista_vazia_se_ctmt_nao_encontrado(self):
        mock_circuitos = MagicMock()
    ...
    def test_retorna_lista_vazia_se_conj_nao_encontrado(self):
        mock_circuitos = MagicMock()
        mock_circuitos.find_one.return_value = {'job_id': JOB_ID, 'records': []}
    ...
    def test_usa_cod_id_como_nome_quando_nome_ausente(self):
...
class TestEndpointPtPnt:
    def test_retorna_200_com_dados(self, client):
    ...
    def test_retorna_404_quando_sem_dados(self, client):
        with _mock_calcular([]):
            response = client.get('/pt-pnt?job_id=nao-existe')

        assert response.status_code == HTTPStatus.NOT_FOUND

    def test_retorna_500_em_erro_inesperado(self, client):
        with patch(
            'backend.routes.pt_and_pnt.calculate_pt_pnt',
    ...
    def test_sem_job_id_retorna_422(self, client):
