...
def render_prophet_forecast(cnpj: str) -> dict:
    ...
    try:
        ...
        logger.warning('[prophet_service] CNPJ inválido, pulando geração de gráficos. cnpj=%s', cnpj)
        return {'sig_agente': None, 'render_paths': {}, 'skipped': _INDICATORS}
    ...
    agente_data = df_hierarchical_agg[df_hierarchical_agg['NumCNPJ'] == cnpj_int]

    if agente_data.empty:
        logger.warning('[prophet_service] CNPJ não encontrado no pickle. cnpj=%s', cnpj)
        return {'sig_agente': None, 'render_paths': {}, 'skipped': _INDICATORS}
